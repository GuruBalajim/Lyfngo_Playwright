...
test('Cancel Appointment From Calendar', async ({ page }, testInfo) => {
...
const login = new LoginPage(page);
  ...
  // ---------------------------
  await calendar.openFirstBookedAppointment();

  // ---------------------------
  // CANCEL APPOINTMENT
  // ---------------------------
   await modal.cancelAppointmentFlow();
 ...
 await expect(page.getByText(/Cancelled/i)).toBeVisible();
  ...
  // ---------------------------
   api.stopCapture();
  await api.saveFiles("Cancel-Appointment-From-Calendar");
});
